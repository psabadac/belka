...
/*
       ...
       \  :  `. `.       ,'-. : | 
        ...
        ;/   :; ;             ,:'     (   ,:) 
         ...
         Telegram: https://t.me/BelkaCoin_TG
*/
...
contract Belka is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "BELKA";
    ...
    uint256 public numTokensSellToAddToLiquidity = 60 * 10**13;
    ...
    function _transferStandard(
        ...
        // _getTValues
        uint256 tFee = tAmount.mul(currentFee).mul(5).div(4000); // 12 * 5/40 = 1.5%
        uint256 tLiquidityFee = tAmount.mul(currentFee).mul(7).div(800); // 12 * 7/8 = 10.5%
    ...
    function setBuyFee(uint256 buyFee) external onlyOwner {
        require(buyFee < 61, "Buy fee must be lower than 61.");
        _buyFee = buyFee;
    ...
    function setSellFee(uint256 sellFee) external onlyOwner {
        require(sellFee < 26, "Sell fee must be lower than 26.");
        _sellFee = sellFee;
